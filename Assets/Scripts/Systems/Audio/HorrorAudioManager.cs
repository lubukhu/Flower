...
public class HorrorAudioManager : MonoBehaviour
{
    ...
    // 🔥 [BUG FIX] Coroutine trùng lắp
    private Coroutine currentGlitchCoroutine;
    private CRTData stableCrtData;
        ...
        while (true)
        {
            ...
            if (soundData.glitchPresets != null && soundData.glitchPresets.Count > 0)
            {
                if (currentGlitchCoroutine != null)
                {
                    ...
                    // Khôi phục Camera về gốc ngay khi bị cắt ngang, nếu không nó sẽ bị lag sọc màn hình vĩnh viễn.
                    if (crtCamera != null && stableCrtData != null)
                    {
                        crtCamera.data = stableCrtData;
                    }
                }
                currentGlitchCoroutine = StartCoroutine(GlitchEffect(soundData, volume));
            }
        }
    }
    ...
    IEnumerator GlitchEffect(HorrorSoundData soundData, float intensity)
    {
        ...
        // 🔥 [BUG FIX] Update stable data
        stableCrtData = crtCamera.data.Clone();
        CRTData original = stableCrtData;
