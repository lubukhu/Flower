    ...
    public class PathFinder
    {
        ...
        public List<OverlayTile> FindPath(OverlayTile start, OverlayTile end, List<OverlayTile> inRangeTiles)
        {
            ...
            while (openList.Count > 0)
            {
                ...
                {
                    if (openList[i].F < currentOverlayTile.F)
                    {
                        currentOverlayTile = openList[i];
                    }
                }
            ...
            {
                GameLogger.LogError($"[PathFinder] Thuật toán tìm đường bị gãy! Lỗi: {ex.Message}");
                return new List<OverlayTile>(); // Trả về list rỗng để nhân vật đứng im, cứu game k bị văng
            }
        }
